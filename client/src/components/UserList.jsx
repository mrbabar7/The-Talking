...
const UserList = ({ searchContact }) => {
  ...
  const filteredConversations = useMemo(() => {
    if (!searchContact) return conversationList;
    const lowercasedFilter = searchContact.trim().toLowerCase();
    return conversationList.filter((contact) => {
      return contact.userName.toLowerCase().includes(lowercasedFilter);
    });
    ...
    <>
      {filteredConversations.length === 0 ? (
        <div className="h-full flex justify-center items-center">
          <h1 className="text-indigo-900 md:p-10 p-7">No contacts available</h1>
        </div>
      ) : (
        <div className="h-full overflow-auto md:scrollbar-indigo scrollbar-fuck flex md:flex-col flex-row md:space-x-0 space-x-4 md:pb-0 pb-2">
          {filteredConversations.map((contact) => {
            ...
            return (
              <div
                key={contact.userId}
                className={`flex gap-3 items-center md:p-3 p-1 md:w-full md:h-auto w-20 h-20 hover-theme ${
                  selectedContact?.userId === contact.userId &&
                  "selected-theme border-2 border-accent rounded-full md:border-0 md:rounded-none"
              ...
              >
                ...
                <div className="hidden md:block flex-1 flex-col space-y-2 items-start py-1 min-w-0">
                  <div className="flex justify-between items-center">
                    <p
                      className="text-sm text-app hover-text font-semibold truncate"
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {contact?.unreadCount > 0 && (
                      <span className="bg-green-600 text-white text-xs rounded-full px-2">
                        {contact?.unreadCount}
                      </span>
                    )}
                  </div>
